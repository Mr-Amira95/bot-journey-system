...
@section('header-actions')
    ...
    @if(auth()->user()->hasPermission('create_expense_categories'))
        ...
        <table class="w-full text-sm">
            <thead class="bg-stone-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Custom Title</th>
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Expenses</th>
                    <th class="text-right px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                @php
                    $canEditCats   = auth()->user()->hasPermission('edit_expense_categories');
                    $canDeleteCats = auth()->user()->hasPermission('delete_expense_categories');
                @endphp
                @forelse($categories as $category)
                    @php
                        $typeColors = [
                            ...
                            'hourly_employees' => 'bg-amber-100 text-amber-700',
                            'other'            => 'bg-slate-100 text-slate-600',
                        ];
                        $tc = $typeColors[$category->type->value] ?? 'bg-slate-100 text-slate-600';
                    @endphp
                    <tr class="hover:bg-stone-50/60 transition-colors">
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-mono font-medium {{ $tc }}">
                                {{ ucwords(str_replace('_', ' ', $category->type->value)) }}
                            </span>
                        </td>
                        <td class="px-5 py-4 text-slate-700 font-mono text-xs">{{ $category->title ?? '—' }}</td>
                        <td class="px-5 py-4 text-slate-500 font-mono text-xs">{{ $category->expenses_count }}</td>
                        <td class="px-5 py-4 text-right">
                            <div class="inline-flex items-center gap-1.5">
                                @if($canEditCats)
                                <button @click="openEdit({ id: {{ $category->id }}, type: '{{ $category->type->value }}', title: '{{ e($category->title ?? '') }}' })"
                                ...
                                @endif
                                @if($canDeleteCats)
                                <button @click="$dispatch('confirm:delete', { action: '{{ route('expense-categories.destroy', $category) }}' })"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-5 py-14 text-center">
                            <p class="text-slate-400 font-mono text-sm">No expense categories yet.</p>
                            @if(auth()->user()->hasPermission('create_expense_categories'))
    ...
    <div x-show="open"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0">
            <h2 class="text-base font-semibold text-slate-800"
                x-text="mode === 'create' ? 'New Category' : 'Edit Category'"></h2>
        ...
        {{-- Form --}}
        <form method="POST"
              :action="mode === 'create' ? '{{ route('expense-categories.store') }}' : '{{ url('expense-categories') }}/' + recordId"
            ...
            <div class="px-6 py-6 space-y-5 flex-1">
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        @foreach($types as $t)
                            <option value="{{ $t->value }}">{{ ucwords(str_replace('_', ' ', $t->value)) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-600 mb-1.5">
                        Custom Title
                        <span class="text-slate-400 font-normal">(optional — overrides the type label)</span>
                    </label>
                    <input type="text" name="title"
                           :value="formData.title"
                           placeholder="e.g. AWS & Cloud Tools"
                           class="w-full text-sm border border-slate-300 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E26B3D]/40 focus:border-[#E26B3D] text-slate-700 placeholder-slate-400 transition-colors">
                </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 shrink-0 flex gap-3">
                <button type="submit"
                        ...
                        x-text="mode === 'create' ? 'Create Category' : 'Save Changes'"></button>
