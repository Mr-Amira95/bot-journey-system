...
<div x-data="{
    ...
    openCreate() {
        ...
        this.formData = { employee_id: '', salary: '', hourly_rate: '', effective_date: '', end_date: '', notes: '' };
        this.open = true;
    },
    openEdit(data) {
        this.mode = 'edit';
    ...
    {{-- Filters --}}
    <div class="mb-5">
        <form method="GET" action="{{ route('salary-histories.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            @if(request()->hasAny(['search', 'employee_id']))
                <a href="{{ route('salary-histories.index') }}" class="text-sm text-slate-500 hover:text-slate-700 font-mono">Clear</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-stone-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Salary</th>
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Hourly Rate</th>
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Effective Date</th>
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">End Date</th>
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Changed By</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($histories as $history)
                    <tr class="hover:bg-stone-50/60 transition-colors">
                        <td class="px-5 py-4">
                            <p class="font-medium text-slate-800">{{ $history->employee->user?->name ?? '—' }}</p>
                            <p class="text-xs text-slate-400 font-mono">{{ $history->employee->position ?? '' }}</p>
                        </td>
                        <td class="px-5 py-4 font-mono text-slate-800">
                            @if($history->salary)
                                ${{ number_format($history->salary, 2) }}
                            @else
                                <span class="text-slate-400">—</span>
                            @endif
                        </td>
                        <td class="px-5 py-4 font-mono text-slate-800">
                            @if($history->hourly_rate)
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="inline-flex items-center gap-1.5">
                                ...
                                <button @click="$dispatch('confirm:delete', { action: '{{ route('salary-histories.destroy', $history) }}' })"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-5 py-14 text-center">
                            <p class="text-slate-400 font-mono text-sm">No salary records yet.</p>
    ...
    <div x-show="open"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0">
            <h2 class="text-base font-semibold text-slate-800"
                x-text="mode === 'create' ? 'Add Salary Record' : 'Edit Salary Record'"></h2>
        ...
        <form method="POST"
              :action="mode === 'create' ? '{{ route('salary-histories.store') }}' : '{{ url('salary-histories') }}/' + recordId"
            ...
            <div class="px-6 py-6 space-y-5 flex-1">
                <div>
                    ...
                    <select name="employee_id"
                            ...
                            :class="submitted && !formData.employee_id ? 'border-red-400' : 'border-slate-300'">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1.5">Monthly Salary</label>
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-mono">$</span>
                            <input type="number" name="salary" step="0.01" min="0" :value="formData.salary"
                                   placeholder="0.00"
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1.5">Hourly Rate</label>
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-mono">$</span>
                            <input type="number" name="hourly_rate" step="0.01" min="0" :value="formData.hourly_rate"
                                   placeholder="0.00"
                                   class="w-full pl-7 pr-3 py-2.5 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E26B3D]/40 focus:border-[#E26B3D] text-slate-700 font-mono transition-colors">
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1.5">Effective Date <span class="text-red-500">*</span></label>
                        <input type="date" name="effective_date" :value="formData.effective_date"
                               ...
                               :class="submitted && !formData.effective_date ? 'border-red-400' : 'border-slate-300'">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" x-effect="$el.value = formData.notes"
                              placeholder="Reason for change, context..."
