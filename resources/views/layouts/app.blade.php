...
$expActive      = request()->routeIs('expense*');
    ...
    // Count accessible items for each group to determine if a group wrapper is needed
    $peopleItemsCount = ($user->hasPermission('view_employees') ? 1 : 0) + ($user->hasPermission('view_clients') ? 1 : 0);
    $workItemsCount   = ($user->hasPermission('view_projects') ? 1 : 0) + ($user->hasPermission('view_tasks') ? 1 : 0);
    $commsItemsCount  = ($user->hasPermission('view_conversations') ? 1 : 0) + ($user->hasPermission('view_calls') ? 1 : 0);
    $financeItemsCount = ($user->hasPermission('view_expenses') ? 1 : 0) + ($user->hasPermission('view_payroll') ? 1 : 0)
                      + ($user->hasPermission('view_salary_histories') ? 1 : 0) + ($user->hasPermission('view_invoices') ? 1 : 0)
                      + ($user->hasPermission('view_recurring_expenses') ? 1 : 0) + ($user->hasPermission('view_project_budgets') ? 1 : 0);
    $hrItemsCount      = ($user->hasPermission('view_leave_requests') ? 1 : 0) + ($user->hasPermission('view_leave_balances') ? 1 : 0)
                      + ($user->hasPermission('view_overtime_requests') ? 1 : 0) + ($user->hasPermission('view_employee_breaks') ? 1 : 0)
                      + ($user->hasPermission('view_leave_types') ? 1 : 0) + ($user->hasPermission('view_work_schedules') ? 1 : 0)
