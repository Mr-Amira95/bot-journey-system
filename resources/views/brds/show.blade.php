    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="space-y-6 lg:sticky lg:top-20">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <p class="text-xs text-slate-400 mb-4">One-page, client-facing use-case summary with KPIs and ROI — generated from this BRD.</p>

                @if($brd->ai_canvas_data)
                <p class="text-xs text-slate-400 mb-3">Last generated {{ $brd->ai_canvas_generated_at?->format('d M Y, H:i') }}</p>
                <div class="flex items-center gap-3">
                    <a href="{{ route('brds.ai-canvas.export', $brd) }}"
                       class="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-[#E26B3D] px-4 py-2 text-xs font-medium text-white hover:bg-[#c85a2f] transition-colors">
                        Download PDF
                    </a>
                    <form method="POST" action="{{ route('brds.ai-canvas.generate', $brd) }}" class="flex-1"
                ...
                @else
                <form method="POST" action="{{ route('brds.ai-canvas.generate', $brd) }}"
