...
<div class="wrapper">
    <div class="header">
        ...
        <div class="header-title">Overtime Request Update</div>
    </div>
    <div class="content">
        <p>Hi {{ $notifiable->name }},</p>
        <p>Your overtime request has been
            <span class="status-badge status-{{ $status }}">{{ $label }}</span>
        </p>

        <div class="info-card">
            <div><strong>Date:</strong> {{ $overtimeRequest->date?->format('d M Y') }}</div>
            <div><strong>Hours:</strong> {{ $overtimeRequest->hours }}h</div>
        </div>

        <a href="{{ $listUrl }}" class="cta">View Overtime Requests &rarr;</a>
