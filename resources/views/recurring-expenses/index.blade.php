...
@section('header-actions')
    @if(auth()->user()->hasPermission('create_recurring_expenses'))
...
<div x-data="{
    ...
    formData: {
        ...
        next_run_date: '{{ old('next_run_date', '') }}',
    ...
    openCreate() {
        ...
        this.formData = { title:'', description:'', category_id:'', project_id:'', amount:'', frequency:'monthly', start_date:'', end_date:'', next_run_date:'', is_active:'1' };
        this.open = true;
    },
    openEdit(data) {
        this.mode = 'edit';
    ...
    {{-- Filters --}}
    <div class="mb-5">
        <form method="GET" action="{{ route('recurring-expenses.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            <select name="frequency" class="py-2 pl-3 pr-8 rounded-lg border border-slate-300 bg-white text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#E26B3D] font-mono">
                <option value="">All Frequencies</option>
                @foreach($frequencies as $f)
                    <option value="{{ $f->value }}" {{ request('frequency') === $f->value ? 'selected' : '' }}>{{ ucfirst($f->value) }}</option>
                @endforeach
            </select>
            <select name="active" class="py-2 pl-3 pr-8 rounded-lg border border-slate-300 bg-white text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#E26B3D] font-mono">
            ...
            @if(request()->hasAny(['search', 'frequency', 'active']))
                <a href="{{ route('recurring-expenses.index') }}" class="text-sm text-slate-500 hover:text-slate-700 font-mono">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <table class="w-full text-sm">
            <thead class="bg-stone-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Frequency</th>
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Next Run</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @php
                    $canEditRE   = auth()->user()->hasPermission('edit_recurring_expenses');
                    $canDeleteRE = auth()->user()->hasPermission('delete_recurring_expenses');
                @endphp
                @forelse($templates as $template)
                    <tr class="hover:bg-stone-50/60 transition-colors">
                        <td class="px-5 py-4">
                            <p class="font-medium text-slate-800">{{ $template->title }}</p>
                            @if($template->project)
                                <p class="text-xs text-slate-400 mt-0.5 font-mono">{{ $template->project->name }}</p>
                            @endif
                        </td>
                        <td class="px-5 py-4 text-xs font-mono text-slate-600">
                            {{ $template->category?->title ?? ucwords(str_replace('_', ' ', $template->category?->type?->value ?? '')) }}
                        </td>
                        <td class="px-5 py-4 font-mono font-medium text-slate-800">${{ number_format($template->amount, 2) }}</td>
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mono font-medium bg-purple-100 text-purple-700">
                                {{ ucfirst($template->frequency->value) }}
                            </span>
                        </td>
                        <td class="px-5 py-4 font-mono text-xs text-slate-600">{{ $template->next_run_date->format('M d, Y') }}</td>
                        <td class="px-5 py-4">
                            @if($template->is_active)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-mono font-medium bg-emerald-100 text-emerald-700">Active</span>
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-mono font-medium bg-slate-100 text-slate-500">Inactive</span>
                            @endif
                        </td>
                        <td class="px-5 py-4 text-right">
                            <div class="inline-flex items-center gap-1.5">
                                ...
                                <button @click="openEdit({
                                            ...
                                            next_run_date: '{{ $template->next_run_date->format('Y-m-d') }}',
                                ...
                                @endif
                                @if($canDeleteRE)
                                <button @click="$dispatch('confirm:delete', { action: '{{ route('recurring-expenses.destroy', $template) }}' })"
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-5 py-14 text-center">
                            <p class="text-slate-400 font-mono text-sm">No recurring expenses set up yet.</p>
                            @if(auth()->user()->hasPermission('create_recurring_expenses'))
                            <button @click="$dispatch('panel:create')" class="mt-3 text-sm text-[#E26B3D] hover:underline font-mono">Add the first template</button>
                            @endif
                        </td>
                    </tr>
                @endforelse
        ...
        @if($templates->hasPages())
            <div class="px-5 py-3 border-t border-slate-100">{{ $templates->links() }}</div>
        @endif
    </div>
    ...
    <div x-show="open"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0">
            <h2 class="text-base font-semibold text-slate-800"
                x-text="mode === 'create' ? 'New Recurring Expense' : 'Edit Recurring Expense'"></h2>
        ...
        <form method="POST"
              :action="mode === 'create' ? '{{ route('recurring-expenses.store') }}' : '{{ url('recurring-expenses') }}/' + recordId"
            ...
            <div class="px-6 py-6 space-y-5 flex-1">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" x-effect="$el.value = formData.description"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="category_id"
                                ...
                                :class="submitted && !formData.category_id ? 'border-red-400' : 'border-slate-300'">
                            <option value="">Select...</option>
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}">{{ $cat->title ?? ucwords(str_replace('_', ' ', $cat->type->value)) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="amount" step="0.01" min="0" :value="formData.amount"
                                   placeholder="0.00"
                                   ...
                                   :class="submitted && !formData.amount ? 'border-red-400' : 'border-slate-300'">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1.5">Frequency <span class="text-red-500">*</span></label>
                        <select name="frequency"
                                x-effect="$el.value = formData.frequency"
                                @change="formData.frequency = $event.target.value"
                                class="w-full text-sm border border-slate-300 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E26B3D]/40 focus:border-[#E26B3D] text-slate-700 bg-white transition-colors">
                            @foreach($frequencies as $f)
                                <option value="{{ $f->value }}">{{ ucfirst($f->value) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date" :value="formData.start_date"
                               class="w-full text-sm border rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E26B3D]/40 focus:border-[#E26B3D] text-slate-700 transition-colors"
                               :class="submitted && !formData.start_date ? 'border-red-400' : 'border-slate-300'">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1.5">End Date <span class="text-slate-400 font-normal">(optional)</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-600 mb-1.5">Next Run Date <span class="text-red-500">*</span></label>
                    <input type="date" name="next_run_date" :value="formData.next_run_date"
                           class="w-full text-sm border rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#E26B3D]/40 focus:border-[#E26B3D] text-slate-700 transition-colors"
                           :class="submitted && !formData.next_run_date ? 'border-red-400' : 'border-slate-300'">
                </div>

                <div x-show="mode === 'edit'" class="flex items-center gap-3">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" id="is_active" value="1"
                           x-effect="$el.checked = formData.is_active === '1' || formData.is_active === true"
                           class="w-4 h-4 rounded border-slate-300 text-[#E26B3D] focus:ring-[#E26B3D]">
                    <label for="is_active" class="text-sm text-slate-700 font-medium cursor-pointer">Active</label>
                </div>
            </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 shrink-0 flex gap-3">
                <button type="submit"
                        ...
                        x-text="mode === 'create' ? 'Create Template' : 'Save Changes'"></button>
