...
@section('header-actions')
    @if(auth()->user()->hasPermission('create_leave_requests'))
...
<div class="flex items-center gap-1 bg-slate-100 rounded-xl p-1 w-fit mb-5">
    <a href="{{ route('leave-requests.index', ['tab' => 'mine']) }}"
    ...
    <a href="{{ route('leave-requests.index', ['tab' => 'all']) }}"
...
<div x-data="{
    ...
    <div class="mb-5">
        <form method="GET" action="{{ route('leave-requests.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            @if(request()->hasAny(['leave_type_id', 'status', 'employee_id']))
                <a href="{{ route('leave-requests.index', ['tab' => $tab]) }}" class="text-sm text-slate-500 hover:text-slate-700 font-mono">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <table class="w-full text-sm">
            <thead class="bg-stone-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Dates</th>
                    <th class="text-left px-5 py-3.5 text-xs font-mono font-medium text-slate-500 uppercase tracking-wider">Days</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                @php
                    ...
                    $canEditLR   = auth()->user()->hasPermission('edit_leave_requests');
                    $canDeleteLR = auth()->user()->hasPermission('delete_leave_requests');
                @endphp
                    ...
                    <tr class="hover:bg-stone-50/60 transition-colors">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                @if($req->leaveType->color)
                                    <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background:{{ $req->leaveType->color }}"></span>
                                @endif
                                <span class="text-slate-700">{{ $req->leaveType->name }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="inline-flex items-center gap-1.5">
                                @if($req->status->value === 'pending')
                                    @if($canApprove)
                                        <form method="POST" action="{{ route('leave-requests.approve', $req) }}" class="inline">
                                            @csrf
                                            <button type="submit" onclick="return confirm('Approve this leave request?')"
                                        ...
                                        <form method="POST" action="{{ route('leave-requests.reject', $req) }}" class="inline">
                                            @csrf
                                            <button type="submit" onclick="return confirm('Reject this leave request?')"
                                ...
                                @endif
                                @if($canDeleteLR)
                                <button @click="$dispatch('confirm:delete', { action: '{{ route('leave-requests.destroy', $req) }}' })"
                ...
                @empty
                    <tr>
                        <td colspan="{{ $tab === 'all' ? 6 : 5 }}" class="px-5 py-14 text-center">
                            <p class="text-slate-400 font-mono text-sm">No leave requests found.</p>
                            @if(auth()->user()->hasPermission('create_leave_requests'))
                            <button @click="$dispatch('panel:create')" class="mt-3 text-sm text-[#E26B3D] hover:underline font-mono">Submit the first one</button>
                            @endif
                        </td>
                    </tr>
                @endforelse
        ...
        @if($leaveRequests->hasPages())
            <div class="px-5 py-3 border-t border-slate-100">{{ $leaveRequests->links() }}</div>
        @endif
    </div>
    ...
    <div x-show="open"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0">
            <h2 class="text-base font-semibold text-slate-800" x-text="mode === 'create' ? 'New Leave Request' : 'Edit Leave Request'"></h2>
        ...
        <form method="POST"
              :action="mode === 'create' ? '{{ route('leave-requests.store') }}' : '{{ url('leave-requests') }}/' + recordId"
            ...
            <div class="px-6 py-6 space-y-6 flex-1">
                <div>
                    <h3 class="text-xs font-mono font-semibold text-slate-400 uppercase tracking-widest mb-4">Request Details</h3>
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="leave_type_id"
                                    ...
                                    :class="submitted && !formData.leave_type_id ? 'border-red-400 ring-1 ring-red-400' : 'border-slate-300'">
